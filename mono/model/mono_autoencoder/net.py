    ...
    def compute_losses(self, inputs, outputs, features):
        ...
        for scale in self.opt.scales:
            ...
            if self.count % interval == 0:
                img_path = os.path.join('test/odo', 'auto_{:0>4d}_{}.png'.format(self.count // interval, scale))
                plt.imsave(img_path, pred[0].transpose(0,1).transpose(1,2).data.cpu().numpy())
                img_path = os.path.join('test/odo', 'img_{:0>4d}_{}.png'.format(self.count // interval, scale))
